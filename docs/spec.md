...
- **Store del cache: Upstash Vector.** Revisión jun 2026 (Día 2, vía Context7): la decisión original decía "Redis con módulo de vectores (RediSearch/Redis Stack)" — Upstash Redis gestionado NO tiene ese módulo; su feature "Search" es full-text (motor Tantivy), no similitud por coseno sobre embeddings. La búsqueda vectorial real en Upstash vive en **Upstash Vector**, producto serverless separado (su propio índice, su propio par de credenciales REST URL/token). Se usa el SDK `upstash-vector` (Python): el índice se crea con `dimension` fijo (igual al modelo de embeddings) y `similarityFunction: COSINE`; la respuesta cacheada se guarda directo en los `metadata` del vector (no hace falta un segundo store). Descartado pgvector por las mismas razones que antes (3ª aparición en el portfolio, no agrega keyword). **Trade-off aceptado:** Upstash Vector no tiene TTL nativo por entrada (a diferencia de Redis) — la expiración de cache entries, si se necesita, se maneja en código vía `created_at` en metadata, o se omite en v1 (no es crítico: el umbral de similitud es el corazón del proyecto, no la expiración).
...
```
- **Contenedor:** Docker. **Hosting:** Railway. **Store:** Upstash Vector (índice de similitud coseno; sin TTL nativo, ver sección 3).

## 6. Modelo de datos mínimo
- **Cache entry** (store): vector de Upstash con `metadata = { prompt_hash, response (texto), model, tokens_in, tokens_out, created_at }`. Sin `ttl` nativo (Upstash Vector no lo soporta) — `created_at` queda para una expiración manual futura si hace falta.
